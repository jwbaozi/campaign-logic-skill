...
For a request that relies on a current meme, hot topic, creator trend, or platform trend, use `workflows/trend-scan.md` before proposing it. Also use it for every substantive review or plan containing a near-term named node/launch, even if the user did not explicitly ask for a meme: preserve a standalone `当下热梗/趋势扫描` section with candidate(s), source, source/capture date, brand-safety decision, and a non-trend fallback. If no candidate passes the checks, say so explicitly; do not silently omit the section. For a future node, state: `在实际发布前 T-14 至 T-7 重新扫描并做平台、权益与品牌安全核验`, and give a durable non-trend route now. Treat a trend as time-bounded evidence; never use one as a substitute for a campaign mechanism. For a visual report, use `workflows/visual-report.md` and the supplied report generator; do not invent charts or turn missing data into a visual claim.
